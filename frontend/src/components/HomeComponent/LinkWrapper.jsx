...
export const LinkWrapper = ({
    ...
    return (
        <div className='fade-in flex flex-col items-center justify-start w-full h-full gap-[0.5rem]'>
            {/* {cards.length >= 1 && (
                <div className='flex items-center justify-between w-full border-b border-dashed border-b-[#FAFAFA]/15 pb-[0.5rem]'>
                    <div className='flex items-center gap-[1rem]'>
                        <span className='text-[#8cd56a] text-[22px] font-bold leading-none'>{selectedGroup}</span>
                        <span className='text-[#FAFAFA] opacity-50 leading-none'>Showing {cards.length} results</span>
                    </div>

                        <button onClick={() => setOpenAddLinkModal(true)}  title='Add Link' type='button' className='flex items-center justify-center bg-[#71cb47]/75 hover:bg-[#71cb47] py-1 active:bg-[#71cb47]/75 rounded-[10px] cursor-pointer'>
                            <Add className="size-6 pl-2"/>
                            <span className='px-1 pr-3 text-[15px] text-[#141414] font-[500] leading-none'>Add a link</span>
                        </button>
                </div>
            )} */}
